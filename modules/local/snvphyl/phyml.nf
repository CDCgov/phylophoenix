...
process PHYML {
    ...
    """
    singularity exec "${container}" phyml -i ${snvAlignment_phy} --datatype nt --model GTR -v 0.0 -s BEST --ts/tv e --nclasses 4 --alpha e --bootstrap -4 --quiet
    ...
    "${task.process}":
        phyml: \$( singularity exec "${container}" phyml --version | grep -oP '[0-9]+\\.[0-9]+\\.[0-9]+' )
