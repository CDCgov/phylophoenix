...
def find_lat_lon(row, data, state_lookup):
    ...
    if state and county and not city:
        ...
        result = data[(data["name"].str.lower() == county.lower()) & (data["admin1_code"].str.lower() == state.lower())]
        if not result.empty:
            return result.iloc[0][["latitude", "longitude"]].values
    ...
    if state and city:
        ...
        result = data[(data["name"].str.lower().isin([city.lower(), city_full_name.lower()])) & (data["admin1_code"].str.lower() == state.lower())]
        if not result.empty:
            return result.iloc[0][["latitude", "longitude"]].values
...
def standardize_location_columns(df):
    ...
    for keyword in keywords:
        ...
        if len(matching_columns) > 1:
            raise ValueError(f"Multiple columns found matching \"{keyword}\": {matching_columns}. Please resolve these duplicates before standardizing.")
        if len(matching_columns) == 1:
            ...
            if keyword in df.columns and matching_columns[0].lower() != keyword:
                raise ValueError(f"Column \"{keyword}\" already exists in the DataFrame, which conflicts with the matched column \"{matching_columns[0]}\". Please resolve this conflict.")
