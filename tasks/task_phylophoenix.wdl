...
task phoenix {
  ...
  command <<<
    ...
    if nextflow run cdcgov/phylophoenix -plugins nf-google@1.1.3 -profile terra -r $version --outdir ./phylophx_output --terra true $input_file ~{true='--by_st' false='' by_st} ~{true='--no_all' false='' no_all} --window_size ~{window_size}--tmpdir $TMPDIR --max_cpus ~{cpu} --max_memory '~{memory}.GB' ; then
  ...
  output {
    ...
    String  busco                             = read_string("BUSCO_PERCENT_MATCH") #seems like I can't make these busco things optional so just leaving blank
